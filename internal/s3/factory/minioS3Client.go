...
func generateMinioClient(url string, accessKey string, secretKey string, region string, caCertificateBase64 []string) (*minio.Client, error) {
	...
	if len(caCertificateBase64) > 0 {
		addTlsClientConfigToMinioOptions(caCertificateBase64, minioOptions)
	}
...
func generateAdminMinioClient(url string, accessKey string, secretKey string, region string, caCertificateBase64 []string) (*madmin.AdminClient, error) {
	hostname, isSSL, err := extractHostAndScheme(url)
...
func addTlsClientConfigToMinioOptions(caCertificatesBase64 []string, minioOptions *minio.Options) {
	rootCAs, _ := x509.SystemCertPool()
	if rootCAs == nil {
		rootCAs = x509.NewCertPool()
	}

	for _, _caCertificateBase64 := range caCertificatesBase64 {
		decodedCaCertificate, err := base64.StdEncoding.DecodeString(_caCertificateBase64)
		if err != nil {
