...
func (r *S3InstanceReconciler) handleReconciliation(ctx context.Context, req reconcile.Request, s3InstanceResource *s3v1alpha1.S3Instance) (reconcile.Result, error) {
	...
	if !found {
		...
		return r.handleS3InstanceCreation(ctx, req, s3InstanceResource)
	}
	logger.Info("this S3Instance already exists and will be reconciled")
	return r.handleS3InstanceUpdate(ctx, req, s3InstanceResource)
}

func (r *S3InstanceReconciler) handleS3InstanceUpdate(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (reconcile.Result, error) {
	logger := log.FromContext(ctx)
	...
	if err != nil {
		logger.Error(err, "Could not get s3Instance auth secret in namespace", "s3InstanceSecretRefName", s3InstanceResource.Spec.SecretRef, "NamespacedName", req.NamespacedName.String())
		return r.SetReconciledCondition(ctx, req, s3InstanceResource, s3v1alpha1.Unreachable,
			"Failed to fetch S3Instance's secret ", err)
	}

	s3InstanceCaCertSecretExpected, err := r.getS3InstanceCaCertSecret(ctx, s3InstanceResource)
	if err != nil {
		logger.Error(err, "Could not get s3Instance cert secret in namespace", "s3InstanceSecretRefName", s3InstanceResource.Spec.SecretRef, "NamespacedName", req.NamespacedName.String())
		return r.SetReconciledCondition(ctx, req, s3InstanceResource, s3v1alpha1.Unreachable,
			"Failed to fetch S3Instance's cert secret ", err)
...
func (r *S3InstanceReconciler) handleS3InstanceCreation(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (reconcile.Result, error) {
	logger := log.FromContext(ctx)
...
func (r *S3InstanceReconciler) handleS3InstanceDeletion(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (reconcile.Result, error) {
	...
	if controllerutil.ContainsFinalizer(s3InstanceResource, s3InstanceFinalizer) {
		...
		if err := r.finalizeS3Instance(ctx, s3InstanceResource); err != nil {
			...
			return r.SetReconciledCondition(ctx, req, s3InstanceResource, s3v1alpha1.DeletionFailure,
...
func (r *S3InstanceReconciler) SetReconciledCondition(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance, reason string, message string, err error) (ctrl.Result, error) {
	...
	if changed {
		if errStatusUpdate := r.Status().Update(ctx, s3InstanceResource); errStatusUpdate != nil {
			logger.Error(errStatusUpdate, "Failed to update GrafanaInstance status", "NamespacedName", req.NamespacedName.String())
...
func (r *S3InstanceReconciler) getS3InstanceAccessSecret(ctx context.Context, s3InstanceResource *s3v1alpha1.S3Instance) (corev1.Secret, error) {
	...
	err := r.Get(ctx, types.NamespacedName{Namespace: s3InstanceResource.Namespace, Name: s3InstanceResource.Spec.SecretRef}, s3InstanceSecret)
	if err != nil {
		if k8sapierrors.IsNotFound(err) {
...
func (r *S3InstanceReconciler) getS3InstanceCaCertSecret(ctx context.Context, s3InstanceResource *s3v1alpha1.S3Instance) (corev1.Secret, error) {
	...
	err := r.Get(ctx, types.NamespacedName{Namespace: s3InstanceResource.Namespace, Name: s3InstanceResource.Spec.CaCertSecretRef}, s3InstanceCaCertSecret)
	if err != nil {
		if k8sapierrors.IsNotFound(err) {
...
func (r *S3InstanceReconciler) checkS3InstanceReferencesInBucket(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (ctrl.Result, error) {
	...
	for _, bucket := range bucketLists.Items {
		if bucket.Spec.S3InstanceRef == s3InstanceResource.Namespace+"/"+s3InstanceResource.Name {
			found++
		}
	}
...
func (r *S3InstanceReconciler) checkS3InstanceReferencesInPolicy(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (ctrl.Result, error) {
	...
	for _, policy := range policyLists.Items {
		if policy.Spec.S3InstanceRef == s3InstanceResource.Namespace+"/"+s3InstanceResource.Name {
			found++
		}
	}
...
func (r *S3InstanceReconciler) checkS3InstanceReferencesInS3User(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (ctrl.Result, error) {
	...
	for _, policy := range s3UserList.Items {
		if policy.Spec.S3InstanceRef == s3InstanceResource.Namespace+"/"+s3InstanceResource.Name {
			found++
		}
	}
...
func (r *S3InstanceReconciler) checkS3InstanceReferencesInPath(ctx context.Context, req ctrl.Request, s3InstanceResource *s3v1alpha1.S3Instance) (ctrl.Result, error) {
	...
	for _, policy := range pathList.Items {
		if policy.Spec.S3InstanceRef == s3InstanceResource.Namespace+"/"+s3InstanceResource.Name {
			found++
		}
	}
