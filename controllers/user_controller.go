...
const (
	userFinalizer = "s3.onyxia.sh/finalizer"
)
...
func (r *S3UserReconciler) handleReconciliation(ctx context.Context, userResource *s3v1alpha1.S3User) (reconcile.Result, error) {
	logger := log.FromContext(ctx)
	s3Client, err := r.getS3InstanceForObject(userResource)
	if err != nil {
...
func (r *S3UserReconciler) getUserSecret(ctx context.Context, userResource *s3v1alpha1.S3User) (corev1.Secret, error) {
	...
	err := r.Get(ctx, types.NamespacedName{Namespace: userResource.Namespace, Name: userResource.Spec.SecretName}, userSecret)
	if err != nil {
		if k8sapierrors.IsNotFound(err) {
...
func (r *S3UserReconciler) getS3InstanceForObject(userResource *s3v1alpha1.S3User) (factory.S3Client, error) {
	return r.S3ClientCache.GetS3Instance(userResource.Name, userResource.Namespace, userResource.Spec.S3InstanceRef)
}
